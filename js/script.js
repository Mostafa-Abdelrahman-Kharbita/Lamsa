...
let orders=[
  {id:'#LM-2847',client:'أميرة حسان',product:'ثريا أورورا الكريستالية',value:'٨٥,٠٠٠ جنيه',status:'proc',date:'١٨ مارس ٢٠٢٤',email:'amira@example.com',phone:'+20 10 1234 5678',desc:'لوبي الفندق، سقف ارتفاعه ٦ أمتار، يحتاج سلسلة مخصصة الطول.'},
  {id:'#LM-2846',client:'كريم إبراهيم',product:'تورشير أطلس × ٤',value:'٧٤,٠٠٠ جنيه',status:'new',date:'١٧ مارس ٢٠٢٤',email:'karim@example.com',phone:'+20 11 9876 5432',desc:'مشروع سكني في الزمالك. التسليم خلال أسبوعين.'},
  {id:'#LM-2845',client:'ليلى منصور',product:'فانوس صحراء × ١٢',value:'١٤٤,٠٠٠ جنيه',status:'done',date:'١٥ مارس ٢٠٢٤',email:'leila@prestige.com',phone:'+971 50 111 2222',desc:'مشروع إضاءة حديقة الفندق.'},
  {id:'#LM-2844',client:'عمر فاروق',product:'إضاءة جدار ميريديان × ٦',value:'٥٣,٤٠٠ جنيه',status:'cancel',date:'١٢ مارس ٢٠٢٤',email:'omar@example.com',phone:'+20 12 5555 6666',desc:'ممر المكاتب، جدران ارتفاعها ٢.٨ متر.'},
  {id:'#LM-2843',client:'نادية علي',product:'مصباح طاولة سيليست × ٣',value:'١٩,٥٠٠ جنيه',status:'done',date:'١٠ مارس ٢٠٢٤',email:'nadia@example.com',phone:'+20 10 7777 8888',desc:'تجديد غرفة النوم في القاهرة الجديدة.'},
];
...
function renderCatalog(prods){
  ...
  if(!el)return;
  el.innerHTML=prods.length?prods.map(productHTML).join(''):`<div style="color:var(--gray);padding:60px;text-align:center;grid-column:1/-1;font-size:18px;font-style:italic">لا توجد منتجات في هذه الفئة بعد.</div>`;
}
...
function addToCart(idOrName){
  const all=getAllProducts();
  const p=typeof idOrName==='number'?all.find(x=>x.id===idOrName):all.find(x=>x.name===idOrName);
  if(!p)return;
...
function updateCartUI(){
  ...
  if(!cart.length){
    ...
    if(tot)tot.style.display='none';return;
  }
...
function updateQty(i,val){cart[i].qty=Math.max(1,parseInt(val)||1);updateCartUI();}
function removeFromCart(i){cart.splice(i,1);updateCartUI();}
function populateProductDropdown(){
  const sel=document.getElementById('quick-product');
  if(!sel)return;
  sel.innerHTML='<option value="">اختر منتجاً للإضافة...</option>'+getAllProducts().map(p=>`<option value="${p.name}">${p.name} — ${fmtP(p.price)}</option>`).join('');
}

function submitOrder(){
  const fname=document.getElementById('fname').value.trim();
  const email=document.getElementById('email').value.trim();
  if(!fname||!email){showNotif('⚠','معلومات ناقصة','من فضلك أدخل اسمك وبريدك الإلكتروني.');return;}
  orders.unshift({
    id:'#LM-'+(2848+orders.length),
    client:fname+' '+(document.getElementById('lname').value||''),
    product:cart.length?cart.map(c=>c.name+(c.qty>1?` × ${c.qty}`:'')).join(', '):'استفسار عام',
    value:cart.length?fmtP(cart.reduce((a,c)=>a+c.price*c.qty,0)):'غير محدد',
    status:'new',date:new Date().toLocaleDateString('ar-EG',{day:'numeric',month:'long',year:'numeric'}),
    email,phone:document.getElementById('phone').value,
    desc:document.getElementById('description').value,
  });
  cart=[];updateCartUI();
  ['fname','lname','email','phone','address','description'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('success-overlay').classList.add('show');
}

const STATUS_MAP={new:'badge-new',proc:'badge-proc',done:'badge-done',cancel:'badge-cancel'};
const STATUS_LABEL={new:'جديد',proc:'قيد المعالجة',done:'مكتمل',cancel:'ملغى'};
...
function renderAdmin(){
  renderDashboardOrders();renderFullOrders();renderAdminProductList();renderMessages();renderAnalytics();
...
function renderDashboardOrders(){
  ...
  tbody.innerHTML=orders.slice(0,5).map(o=>`<tr>
    ...
    <td style="color:var(--gray)">${o.date}</td></tr>`).join('');
}
...
function updateOrderStatus(i,status){
  orders[i].status=status;renderDashboardOrders();
  document.getElementById('admin-msg-count').textContent=orders.filter(o=>o.status==='new').length;
...
function renderAdminProductList(){
  ...
  if(!el)return;
  el.innerHTML=getAllProducts().map((p,i)=>`
...
function removeCustomProduct(i){
  customProducts.splice(i,1);renderAdminProductList();
  document.getElementById('admin-prod-count').textContent=getAllProducts().length;
  document.getElementById('prod-list-count').textContent=getAllProducts().length;
...
function saveProduct(){
  ...
  if(!name||!cat||!price){showNotif('⚠','حقول ناقصة','من فضلك أدخل الاسم والفئة والسعر.');return;}
  ...
  customProducts.push({
    ...
    emoji:'💡',imageUrl:imgEl?imgEl.dataset.dataurl:null,
    material:document.getElementById('prod-material').value,
    dims:document.getElementById('prod-dims').value,
  });
  renderAdminProductList();renderHomeFeatured();
  document.getElementById('admin-prod-count').textContent=getAllProducts().length;
  document.getElementById('prod-list-count').textContent=getAllProducts().length;
  clearProductForm();
  showNotif('✦','تم حفظ المنتج!',name+' أُضيف إلى الكتالوج');
}
function clearProductForm(){
  ['prod-name','prod-price','prod-sale','prod-material','prod-dims','prod-desc','prod-badge'].forEach(id=>document.getElementById(id).value='');
